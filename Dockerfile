...
# ############################################################
# # TARGET 1 — FULL PIPELINE (ingestion + streaming + ml)
# ############################################################
...
# ############################################################
# # TARGET 3 — FASTAPI BACKEND
# ############################################################
# FROM base AS backend
# WORKDIR /app

# # Copy model if pre-trained OR user can train via trainer image
# # You must run trainer first before backend:
# #   docker build --target trainer -t trainer .
# #   docker run --rm trainer
# # Then backend sees model in /app/model/spark_gbt_model

# EXPOSE 9998
# CMD ["uvicorn", "src.backend_fastapi:app", "--host", "0.0.0.0", "--port", "9998"]
